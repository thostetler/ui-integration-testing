...
test('Add/Remove Libraries', async ({page}) => {
...
test('Add/Remove Bibcodes to Library', async ({page}) => {
  ...
  const { number_added: papers } = await page.waitForResponse('**/biblib/documents/*', {timeout: API_TIMEOUT})
  ...
  await page.locator('#library-select').selectOption({label: state.name});
  await page.locator('button.submit-add-to-library').click();
  await expect(page.waitForResponse('**/biblib/documents/*', {timeout: API_TIMEOUT}).then(res => res.json()))
...
test('Collaboration/Visibility works with libraries', async ({browser}) => {
  test.slow();
  ...
  const assertBiblibPermissions = async (page: Page, expected: Array<{ [email: string]: string[] }>) => {
    const permissionsResponse = await page.waitForResponse((res) => {
  ...
  await page.goto(`/user/libraries/${libraryData.id}`);
  expect(await page.locator('button[data-target="#library-actions"]').textContent()).toContain('You have write privileges');
  ...
  await page.reload();
  expect(await page.locator('button[data-target="#library-actions"]').textContent()).toContain('You have admin privileges');
