...
export class ScixUser implements User {
  ...
  async login() {
    ...
    await expect(async () => {
      ...
      await this.page.getByRole('textbox', { name: 'email@example.com' }).fill(this._email);
      await this.page.getByRole('textbox', { name: '*******' }).fill(this._password);
      await this.page.getByRole('button', { name: 'Submit' }).click();
      res = await this.page.waitForResponse('**/api/auth/login');
  ...
  async logout() {
    ...
    const res = await this.page.waitForResponse('**/api/auth/logout');
